...
```{r}
ggplot(data,aes(Area))+geom_histogram(stat="count")+theme_bw()+coord_flip()
```
...
```{r}
data <- data %>% filter(Area=="Portland")
```
...
There is no evidence the probability of a tree being classified as thinning is not related to urban heat.  
