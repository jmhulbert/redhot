...
```{r}
#data$field.dieback.prop[data$field.dieback.prop==1] <- 0.999 # see below discussion - glmmTMB does not currently support zero-one inflated distributions
```
...
#summary(data$field.dieback.prop)
#with(data,any(field.dieback.prop<0|field.dieback.prop>1)) # checks if there are any values <0 or >1
```
...
```{r}
#drop1(zibeta.daily,test="Chi") # need to check what this actually did, but was included in Zurr code
summary(zibeta.daily)
```
...
```{r}
wa.data <- data %>% filter(Poverty!="NA")
```

### Temperature and EHD Rank

```{r}
wa.zibeta.af.ehd <- glmmTMB(field.dieback.prop~ dist.from.mean.af + Environm_1 + (1|Area),data=wa.data,ziformula=~dist.from.mean.af + Environm_1,family=beta_family())
```

```{r}
summary(wa.zibeta.af.ehd)
```

### Temperature and Poverty

```{r}
wa.zibeta.af.pov <- glmmTMB(field.dieback.prop~ dist.from.mean.af + Poverty + (1|Area),data=wa.data,ziformula=~dist.from.mean.af + Poverty,family=beta_family())
```


```{r}
summary(wa.zibeta.af.pov)
```
...
```{r}
zibeta.af.holc <- glmmTMB(field.dieback.prop~ dist.from.mean.af + grade + (1|Area),data=holc.data,ziformula=~dist.from.mean.af + Poverty,family=beta_family())
```


```{r}
summary(zibeta.af.holc)
```
