...
new.data.2.inf <- expand.grid(
  dist.from.mean.af = seq(min(data$dist.from.mean.af), max(150), length.out = 1000), Area = 0)  # keeping predictor2 constant at its mean
```
...
```{r}
p2 <- ggplot(new.data.2.inf, aes(x = dist.from.mean.af, y = predicted_response)) + geom_line() + labs(x = "Distance from mean (F)", y = "Dieback Probability") + scale_x_continuous(limits = c(80, 115)) +theme_minimal()
